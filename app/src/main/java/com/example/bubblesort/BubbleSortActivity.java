...
public class BubbleSortActivity extends AppCompatActivity {
        ...
        private void performBubbleSort(String inputStr) {
            ...
            if (Arrays.asList(inputStr).contains(word)) {
                finish();
                return;
            }
            ...
            for (int i = 0; i < n - 1; i++) {
                ...
                for (int j = n - 1; j > i; j--) {
                    if (arr[j - 1] > arr[j]) {
                        ...
                        highlightNumbers(highlightedString, arr[j - 1], arr[j]);
        ...
        private void highlightNumbers(SpannableString string, int num1, int num2) {
...
private void exitApplication(View v) {
        finish();
    }
}